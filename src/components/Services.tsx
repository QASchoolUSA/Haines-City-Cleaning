    ...
    <section id="services" className="bg-white">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between">
          ...
          <Link href="#booking" className="hidden rounded-full bg-purple-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-purple-700 sm:inline-block">Get a quote</Link>
        </div>
          ...
          {services.map((s) => (
            <Link key={s.key} href={s.href} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md">
              <div className="flex items-center gap-3">
                <div className="grid h-10 w-10 place-items-center rounded-full bg-purple-50 text-lg">{s.icon}</div>
              ...
              <span className="mt-4 inline-block text-sm font-semibold text-purple-700">Learn more →</span>
            </Link>
          ))}
        </div>
      </div>
